...
import {calculateDemoQuote, demoQuote, versionIdForRoute, type DemoQuote} from "@/lib/demoData";
...
export default function QuotePage({params}: PageProps) {
  ...
  const recalculate = async (nextOptionalIds = selectedOptionalIds) => {
    ...
    const fallback = calculateDemoQuote(nextOptionalIds, discountMinor, taxMinor);
    setQuote(fallback);
