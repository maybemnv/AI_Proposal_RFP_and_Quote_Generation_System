...
1. Start Postgres if using the database-backed path: `docker compose up -d db`.
...
4. Install the web dependencies: `cd app/web && npm install`.
5. Copy `.env.example` to `.env` and leave `GENERATION_MODE=fixture` for deterministic demos.

## Before every demo

1. From the repository root, run `python -m app.cli reset`.
2. In terminal 1, run `uvicorn app.api.main:app --reload`.
3. In terminal 2, run `cd app/web && npm run dev`.
4. Run the smoke path: `cd app/web && npm run e2e -- e2e/trace-a.spec.ts`.
5. If the smoke test is green, open `http://localhost:3000`.
...
| 6 | `/proposals/prop_northwind/quote` | Discount authority is enforced, not requested. | Enter `9000.00`, recalculate, and show `Review required`. |
